...
export default function ReserveForm({ itemId, itemTitle }: Props) {
  ...
  if (sent) {
    return (
      <div className="py-6">
        <p className="font-mono text-sm text-teal">✓ Your enquiry has been sent.</p>
        <p className="font-mono text-xs text-stone-grey mt-1">We&apos;ll be in touch within 24 hours.</p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
      <div className="grid grid-cols-1 gap-4">
        <div>
          <label className={labelCls} htmlFor="res-name">Name *</label>
          <input
            id="res-name"
            type="text"
          ...
          />
          {errors.name && (
            <p className="archive-label text-[0.58rem] text-rose-magenta mt-1">{errors.name.message}</p>
          )}
        </div>
      ...
      >
        {isSubmitting ? 'Sending...' : 'Reserve this piece →'}
      </button>
    </form>
  )
}
