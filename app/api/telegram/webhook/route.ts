...
async function handleInitialPhoto(message: TelegramMessage, updateId: number | undefined) {
  ...
  const rawUrl = await uploadCatalogBytes(sourceStoragePath, file.buffer, file.contentType, true)
...
async function handleCloseupPhoto(message: TelegramMessage, session: ConversationSession) {
  ...
  await uploadCatalogBytes(sourceStoragePath, file.buffer, file.contentType, true)
  await enqueueJob({
    jobType: 'closeup',
