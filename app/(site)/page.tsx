    ...
    <>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-near-black">
        <div className="relative z-10 text-center px-6 max-w-4xl">
          ...
          <h1
            className="font-display text-white leading-[0.92] tracking-[-0.035em] mb-6 animate-fade-up"
            style={{ fontSize: 'clamp(3.5rem, 12vw, 8rem)', animationDelay: '300ms' }}
          >
            Buena<br />Onda
          </h1>
          <p
            className="font-display italic text-stone-grey mb-10 animate-fade-up"
            style={{ fontSize: 'clamp(1rem, 2.5vw, 1.4rem)', animationDelay: '450ms' }}
          >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 animate-fade-up" style={{ animationDelay: '600ms' }}>
            <Link
              href="/culture"
              className="px-7 py-3 bg-warm-sand text-white font-mono text-xs tracking-[0.2em] uppercase hover:bg-burnished transition-colors duration-200 paper-hover inline-block"
            >
              Read Culture
            </Link>
            <Link
              href="/radio"
              className="px-7 py-3 border border-white/30 text-white font-mono text-xs tracking-[0.2em] uppercase hover:border-warm-sand hover:text-warm-sand transition-all duration-200 inline-block"
            >
              Listen Now
            </Link>
          </div>
        ...
        <div className="max-w-site mx-auto px-5 md:px-10">
          <div className="grid md:grid-cols-2 gap-16 items-start">
            <ScanReveal>
              ...
              <h2 className="font-display text-near-black mb-6" style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)' }}>
              ...
              <Link href="/about" className="inline-flex items-center gap-2 font-mono text-xs tracking-[0.18em] uppercase text-warm-sand hover:text-burnished transition-colors mt-8 group">
        ...
        <div className="max-w-site mx-auto px-5 md:px-10">
          <div className="flex items-end justify-between mb-12">
            <div>
              ...
              <h2 className="font-display text-near-black" style={{ fontSize: 'clamp(1.6rem, 3.5vw, 2.4rem)' }}>From the house</h2>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {cultureTeaser.map(({ label, issue, title, excerpt, href }, i) => (
              <ScanReveal key={title} delay={i * 100}>
                <Link href={href} className="group block paper-hover bg-white p-6 h-full">
                  ...
                  <h3 className="font-display text-near-black mb-3 group-hover:text-warm-sand transition-colors" style={{ fontSize: 'clamp(1.1rem, 2vw, 1.35rem)' }}>{title}</h3>
        ...
        <div className="max-w-site mx-auto px-5 md:px-10 relative z-10">
          <ScanReveal>
            <div className="max-w-2xl">
              ...
              <h2 className="font-display text-white mt-2 mb-6" style={{ fontSize: 'clamp(1.8rem, 4vw, 3rem)' }}>The signal is always on.</h2>
              <p className="text-stone-grey text-base leading-relaxed mb-8">Curated mixes, live sessions, and field recordings. Music the way it was meant to be heard — in sequence, without shuffle.</p>
              <Link href="/radio" className="inline-flex items-center gap-2 px-6 py-3 border border-warm-sand/40 text-warm-sand font-mono text-xs tracking-[0.2em] uppercase hover:bg-warm-sand/10 hover:border-warm-sand transition-all duration-200 group">
                Enter the archive <span className="group-hover:translate-x-1 transition-transform">→</span>
              </Link>
            </div>
          </ScanReveal>
        </div>
          ...
          <ScanReveal>
            <div className="flex items-end justify-between mb-4">
              <div>
                ...
                <h2 className="font-display text-near-black" style={{ fontSize: 'clamp(1.6rem, 3.5vw, 2.4rem)' }}>Themes.</h2>
              </div>
                ...
                <ScanReveal key={theme.id} delay={i * 80}>
                  ...
                  <Link href={`/themes/${theme.slug}`} className="group block">
                    <div className="grid md:grid-cols-[auto_1fr_auto] gap-6 md:gap-10 py-6 items-center">
                      <span className="catalog-ordinal text-warm-sand/60">{String(i + 1).padStart(2, '0')}</span>
                      <div className="flex items-baseline gap-4 flex-wrap">
                        <span className="catalog-ordinal text-stone-grey">{theme.code}</span>
                        <h3 className="font-display text-near-black group-hover:text-warm-sand transition-colors" style={{ fontSize: 'clamp(1.2rem, 2.5vw, 1.7rem)' }}>{theme.title}</h3>
                        {theme.description && <span className="font-mono text-xs text-stone-grey hidden md:inline">— {theme.description.slice(0, 60)}{theme.description.length > 60 ? '…' : ''}</span>}
                      </div>
                      <span className="text-cta group-hover:text-warm-sand transition-colors">Enter →</span>
                    </div>
                  </Link>
                </ScanReveal>
              ))}
                  ...
                  <Link href="/themes" className="group block">
                    <div className="grid md:grid-cols-[auto_1fr_auto] gap-6 md:gap-10 py-6 items-center">
                      ...
                      <div className="flex items-baseline gap-4 flex-wrap">
                        ...
                        <h3 className="font-display text-near-black/40 group-hover:text-near-black/60 transition-colors" style={{ fontSize: 'clamp(1.2rem, 2.5vw, 1.7rem)' }}>{label}</h3>
          ...
          <ScanReveal>
            <div className="flex items-end justify-between mb-4">
              <div>
                ...
                <h2 className="font-display text-near-black" style={{ fontSize: 'clamp(1.6rem, 3.5vw, 2.4rem)' }}>Recently listed.</h2>
              </div>
              <Link href="/themes" className="text-cta hidden sm:inline-block">Full catalog →</Link>
            </div>
          </ScanReveal>
                ...
                <ScanReveal key={item.id} delay={i * 60}>
                  ...
                  <Link href={`/items/${item.slug}`} className="group block">
                    <div className="grid grid-cols-[4.5rem_1fr_auto] md:grid-cols-[6rem_1fr_auto] gap-5 md:gap-10 py-6 items-center">
                      <div className="aspect-[3/4] overflow-hidden bg-off-white flex-shrink-0">
                      ...
                      <div>
                        {item.catalog_number && <span className="catalog-ordinal text-stone-grey block mb-2">{item.catalog_number}</span>}
                        <p className="font-display text-near-black group-hover:text-warm-sand transition-colors" style={{ fontSize: 'clamp(1rem, 2vw, 1.35rem)', lineHeight: 1.1 }}>{item.title}</p>
                      </div>
                      <div className="text-right">
                        {item.price ? <span className="catalog-ordinal">${item.price.toFixed(2)}</span> : <span className="catalog-ordinal text-stone-grey/30">—</span>}
                      </div>
                    </div>
                  </Link>
                </ScanReveal>
            ...
            <>
              ...
              <ScanReveal>
                <div className="py-20 text-center">
                  ...
                  <Link href="/themes" className="text-cta mt-6 inline-block">Browse themes →</Link>
                </div>
              </ScanReveal>
              <div className="editorial-rule" />
            </>
