...
function makeWaveform(seedStr, n = 150) {
...
function makeShapes(seedStr) {
  const rnd = seeded(seedStr);
  const palette = [C.teal, C.pink, C.coral, C.cyan];
  const kinds = ["tri", "dot", "sq", "ring"];
  const out = [];
...
/* ---------- waveform scrubber ---------- */
function Waveform({ peaks, progress, onSeek, height = 64 }) {
...
/* ---------- main ---------- */
export default function BuenaOndaArchive() {
  ...
  const step = (dir) => {
    const ni = idx + dir;
    if (ni >= 0 && ni < EPISODES.length) {
      setCurrentId(EPISODES[ni].id);
      setPlaying(true);
    }
  };
  ...
  return (
    <div style={{ background: C.ink, color: C.cream, fontFamily: F.head }} className="min-h-screen w-full">
      ...
      <audio
        ...
        onEnded={() => (idx < EPISODES.length - 1 ? step(1) : setPlaying(false))}
        preload="metadata"
      />
          ...
          <div className="flex flex-col sm:flex-row gap-6">
            ...
            <div className="flex-1 min-w-0 flex flex-col">
              ...
              <div className="flex items-center gap-4 mt-4">
                ...
                <button onClick={() => step(1)} disabled={idx === EPISODES.length - 1} aria-label="Next episode"
                  style={{ color: idx === EPISODES.length - 1 ? C.creamFaint : C.cream, background: "none", border: "none", cursor: idx === EPISODES.length - 1 ? "default" : "pointer", padding: 6 }}>
                  <SkipForward size={20} />
                </button>
